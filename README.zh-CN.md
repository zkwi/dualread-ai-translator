...
- **动态页面支持**：在滚动信息流、展开内容、原文更新和 React 式重渲染中复用同一翻译记录，避免重复请求。
- **结构感知排版**：在普通块、表格单元格、列表、Flex/Grid 行、裁剪预览和 Web Component 中安全插入译文，不改写宿主布局样式。
- **响应式阅读样式**：针对密集卡片、移动端和长文章自适应间距，并尊重 RTL 与纵向书写模式。
...
API Key：你自己的服务商密钥
模型：例如 gpt-4o-mini 或 deepseek-v4-flash
...
设置页默认只显示 API 连接、翻译语言、自动翻译和展示方式。界面语言、自定义接口、Thinking 手动覆盖、提示词、并发和维护操作收纳在默认折叠的 **高级设置** 中；选择“自定义”服务商时会自动展开。

界面语言默认跟随浏览器，也可在高级设置中修改；翻译语言独立设置。你可以进行英译中、日译繁中，或使用服务商支持的其他翻译方向。

强烈建议在 **高级设置** 中保持 **自动关闭可控思考模式** 开启。思考 / 推理模式会让翻译明显变慢。默认的 **自动选择** 不根据服务商名称写死参数；点击 **测试 API** 时会尝试通用控制参数并保存可用结果。API 地址或模型变化后需要重新测试，测试前不会发送额外 Thinking 参数。
...
可以，而且为了翻译速度强烈建议开启。保持 **强烈推荐开启：自动关闭可控思考模式** 开启，并优先使用 **自动选择**，然后点击 **测试 API**。DualRead 只会在接口明确表示某个 Thinking 字段不受支持时尝试下一种；鉴权、限流、模型、网络或超时错误会立即停止。也可以在高级设置中手动指定参数。
...
可以。界面语言默认跟随浏览器，也可在高级设置中修改，且独立于源语言和目标语言；扩展 UI 已包含简体中文、繁體中文、英语和日语。
